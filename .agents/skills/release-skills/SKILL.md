---
name: release-skills
description: Universal release workflow. Auto-detects version files and changelogs. Supports Node.js, Python, Rust, Codex Plugin, and generic projects. Use when user says "release", "发布", "new version", "bump version", "push", "推送".
---
...
2. Auto-detect version file by scanning (priority order):
   ...
   - `Cargo.toml` (Rust)
   - `.codex-plugin/plugin.json` or `marketplace.json` (Codex Plugin or plugin marketplace catalog)
...
2. **Group by skill/module**:
   ...
   - Root files (AGENTS.md, README files, plugin manifests, etc.) → Group as "project"
...
**Example Commits**:
```bash
git commit -m "feat(sweety-cover-image): add watercolor and minimalist styles"
git commit -m "fix(sweety-comic): improve panel layout for long dialogues"
...
```
Commits created:
  1. feat(sweety-cover-image): add watercolor and minimalist styles
  2. fix(sweety-comic): improve panel layout for long dialogues
...
Changes grouped by skill/module:
  sweety-cover-image:
    ...
    → Commit: feat(sweety-cover-image): add watercolor and minimalist styles
    → README updates: options table

  sweety-comic:
    - fix: panel layout for long dialogues
    → Commit: fix(sweety-comic): improve panel layout for long dialogues
    → No README updates
