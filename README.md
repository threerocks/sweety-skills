...
Skills shared by Sweety for improving daily work efficiency with Codex.
...
Codex discovers plugins through a local marketplace file. Clone this repo into your user plugin directory:

```bash
mkdir -p ~/plugins ~/.agents/plugins
git clone https://github.com/threerocks/sweety-skills.git ~/plugins/sweety-skills
```
...
Restart Codex after updating the marketplace file. If you already have a marketplace file, append the `sweety-skills` plugin entry instead of replacing your existing list.

### Install Individual Skills

If you only need a few skills rather than the full plugin, install them with the Codex skills installer:

> Install the `sweety-image-gen` skill from github.com/threerocks/sweety-skills

### Publish to ClawHub / OpenClaw

This repository also supports publishing each `skills/sweety-*` directory as an individual ClawHub skill.
...
ClawHub installs skills individually, not as one Codex plugin bundle. After publishing, users can install specific skills such as:

```bash
clawhub install sweety-image-gen
clawhub install sweety-markdown-to-html
```
...
The Codex plugin exposes the repo-level `skills/` directory so each skill is registered exactly once.

| Plugin | Description | Includes |
|--------|-------------|----------|
| **sweety-skills** | Content generation, AI backends, and utility tools for daily work efficiency | All skills in this repository, organized below as Content Skills, AI Generation Skills, and Utility Skills |
...
```bash
cd ~/plugins/sweety-skills
git pull
```

Restart Codex if the plugin list does not refresh automatically.
...
/sweety-xhs-images posts/ai-future/article.md --style tech --layout list

# Direct content input
/sweety-xhs-images 今日星座运势
```
...
# Specify layout
/sweety-infographic path/to/content.md --layout pyramid

# Specify style (default: craft-handmade)
/sweety-infographic path/to/content.md --style technical-schematic

# Specify both
/sweety-infographic path/to/content.md --layout funnel --style corporate-memphis

# With aspect ratio (named preset or custom W:H)
/sweety-infographic path/to/content.md --aspect portrait
...
/sweety-cover-image path/to/article.md --quick

# Specify dimensions (5D system)
/sweety-cover-image path/to/article.md --type conceptual --palette cool --rendering digital
/sweety-cover-image path/to/article.md --text title-subtitle --mood bold

# Style presets (backward-compatible shorthand)
/sweety-cover-image path/to/article.md --style blueprint

# Specify aspect ratio (default: 16:9)
/sweety-cover-image path/to/article.md --aspect 2.35:1

# Visual only (no title text)
/sweety-cover-image path/to/article.md --no-title
```
...
```bash
# From markdown file
/sweety-slide-deck path/to/article.md

# With style and audience
/sweety-slide-deck path/to/article.md --style corporate
/sweety-slide-deck path/to/article.md --audience executives

# Target slide count
/sweety-slide-deck path/to/article.md --slides 15

# Outline only (no image generation)
/sweety-slide-deck path/to/article.md --outline-only

# With language
/sweety-slide-deck path/to/article.md --lang zh
```
...
/sweety-comic posts/turing-story/source.md

# Specify art style and tone
/sweety-comic posts/turing-story/source.md --art manga --tone warm
/sweety-comic posts/turing-story/source.md --art ink-brush --tone dramatic

# Use preset (includes special rules)
/sweety-comic posts/turing-story/source.md --style ohmsha
/sweety-comic posts/turing-story/source.md --style wuxia

# Specify layout and aspect ratio
/sweety-comic posts/turing-story/source.md --layout cinematic
/sweety-comic posts/turing-story/source.md --aspect 16:9

# Specify language
/sweety-comic posts/turing-story/source.md --lang zh

# Direct content input
/sweety-comic "The story of Alan Turing and the birth of computer science"
```
...
```bash
# Post with text
/sweety-post-to-x "Hello from Codex!"

# Post with images
/sweety-post-to-x "Check this out" --image photo.png
...
```bash
/sweety-post-to-wechat 贴图 --markdown article.md --images ./photos/
/sweety-post-to-wechat 贴图 --markdown article.md --image img1.png --image img2.png --image img3.png
...
```bash
# Add to .sweety-skills/.env (project-level) or ~/.sweety-skills/.env (user-level)
...
```bash
mkdir -p .sweety-skills/sweety-post-to-wechat
```

Create `.sweety-skills/sweety-post-to-wechat/EXTEND.md`:
...
```bash
# Post with text
/sweety-post-to-weibo "Hello Weibo!"

# Post with images
/sweety-post-to-weibo "Check this out" --image photo.png

# Post with video
/sweety-post-to-weibo "Watch this" --video clip.mp4
```
...
```bash
# Publish article
/sweety-post-to-weibo --article article.md

# With cover image
/sweety-post-to-weibo --article article.md --cover cover.jpg
```
...
/sweety-image-gen --prompt "A cute cat" --image cat.png

# With aspect ratio
/sweety-image-gen --prompt "A landscape" --image landscape.png --ar 16:9

# High quality (2k)
/sweety-image-gen --prompt "A banner" --image banner.png --quality 2k

# Specific provider
/sweety-image-gen --prompt "A cat" --image cat.png --provider openai

# OpenRouter
/sweety-image-gen --prompt "A cat" --image cat.png --provider openrouter

# DashScope (Aliyun Tongyi Wanxiang)
/sweety-image-gen --prompt "一只可爱的猫" --image cat.png --provider dashscope

# Replicate
/sweety-image-gen --prompt "A cat" --image cat.png --provider replicate

# Jimeng (即梦)
/sweety-image-gen --prompt "一只可爱的猫" --image cat.png --provider jimeng

# Seedream (豆包)
/sweety-image-gen --prompt "一只可爱的猫" --image cat.png --provider seedream

# With reference images (Google, OpenAI, OpenRouter, Replicate, or Seedream 5.0/4.5/4.0)
/sweety-image-gen --prompt "Make it blue" --image out.png --ref source.png
```
...
```bash
/sweety-danger-gemini-web "Hello, Gemini"
/sweety-danger-gemini-web --prompt "Explain quantum computing"
```

**Image Generation:**

```bash
/sweety-danger-gemini-web --prompt "A cute cat" --image cat.png
/sweety-danger-gemini-web --promptfiles system.md content.md --image out.png
```
...
Writing skill for drafting articles in Liu Lei's voice (hands-on engineer, cost-aware, anti-marketing, boundary-honest). Provides a persona anchor, value baseline, a signature lexicon and paired examples extracted from real authorized corpus, an AI-cliché red-line list, and a persona self-check. It is the single entrypoint for the writing pipeline and orchestrates external `shuorenhua` + `humanizer-zh` review before `sweety-four-layer-review`. For WeChat publishing it outputs a scaffold/rough draft or cleanup plan for the author to finish in their own words, since reliably passing AI detectors requires genuinely human text.

#### sweety-four-layer-review

Pre-publish four-layer review (facts/logic → residual AI-flavor → rhythm → does-it-sound-like-the-author), final stage of the pipeline. Only judges and routes issues back upstream (boundary/voice cleanup to external `shuorenhua` / `humanizer-zh`, voice to `sweety-liulei-writer`); it does not rewrite or invent new rules. Outputs a structured QC report with a verdict and fix priorities.
...
/sweety-youtube-transcript https://www.youtube.com/watch?v=VIDEO_ID

# Specify languages (priority order)
/sweety-youtube-transcript https://youtu.be/VIDEO_ID --languages zh,en,ja

# With chapters and speaker identification
/sweety-youtube-transcript https://youtu.be/VIDEO_ID --chapters --speakers

# SRT subtitle format
/sweety-youtube-transcript https://youtu.be/VIDEO_ID --format srt

# List available transcripts
/sweety-youtube-transcript https://youtu.be/VIDEO_ID --list
```
...
/sweety-url-to-markdown https://example.com/article

# Wait mode - for login-required pages
/sweety-url-to-markdown https://example.com/private --wait

# Save to specific file
/sweety-url-to-markdown https://example.com/article -o output.md
```
...
#### sweety-danger-x-to-markdown

Converts X (Twitter) content to markdown format. Supports tweet threads and X Articles.

```bash
# Convert tweet to markdown
/sweety-danger-x-to-markdown https://x.com/username/status/123456

# Save to specific file
/sweety-danger-x-to-markdown https://x.com/username/status/123456 -o output.md

# JSON output
/sweety-danger-x-to-markdown https://x.com/username/status/123456 --json

# Download media (images/videos) to local files
/sweety-danger-x-to-markdown https://x.com/username/status/123456 --download-media
```
...
#### sweety-compress-image

Compress images to reduce file size while maintaining quality.

```bash
/sweety-compress-image path/to/image.png
/sweety-compress-image path/to/images/ --quality 80
```

#### sweety-format-markdown

Format plain text or markdown files with proper frontmatter, titles, summaries, headings, bold, lists, and code blocks.

```bash
# Format a markdown file
/sweety-format-markdown path/to/article.md

# Format with specific output
/sweety-format-markdown path/to/draft.md
```
...
1. CLI environment variables (e.g., `OPENAI_API_KEY=xxx /sweety-image-gen ...`)
2. `process.env` (system environment)
3. `<cwd>/.sweety-skills/.env` (project-level)
4. `~/.sweety-skills/.env` (user-level)

**Setup**:

```bash
# Create user-level config directory
mkdir -p ~/.sweety-skills

# Create .env file
cat > ~/.sweety-skills/.env << 'EOF'
# OpenAI
...
```bash
mkdir -p .sweety-skills
# Add .sweety-skills/.env to .gitignore to avoid committing secrets
echo ".sweety-skills/.env" >> .gitignore
```
...
1. `.sweety-skills/<skill-name>/EXTEND.md` - Project-level (for team/project-specific settings)
2. `~/.sweety-skills/<skill-name>/EXTEND.md` - User-level (for personal preferences)

**Example**: To customize `sweety-cover-image` with your brand colors:

```bash
mkdir -p .sweety-skills/sweety-cover-image
```

Then create `.sweety-skills/sweety-cover-image/EXTEND.md`:
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /sweety-danger-gemini-web "Hello"
```
...
[![Star History Chart](https://api.star-history.com/svg?repos=sweety/sweety-skills&type=Date)](https://www.star-history.com/#sweety/sweety-skills&Date)
