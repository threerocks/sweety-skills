...
**Execution choice**:
- If multiple illustrations already have saved prompt files and the task is now plain generation, prefer `sweety-image-gen` batch mode (`build-batch.ts` -> `main.ts --batchfile`)
...
|------------------------|--------|
| Yes (e.g., sweety-image-gen with Google) | Pass reference images via `--ref` |
