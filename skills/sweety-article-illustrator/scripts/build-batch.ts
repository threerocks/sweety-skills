...
Options:
  ...
  --provider <name>    Provider for sweety-image-gen batch tasks (default: auto-detect with fallback order)
  --model <id>         Model for sweety-image-gen batch tasks (default: current provider default)
...
async function main(): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (args.provider) taskConfig.provider = args.provider;
    if (args.model) taskConfig.model = args.model;
