...
async function generateWithGemini(
  ...
  console.log(
    `Generating image with Gemini${proxy ? ` via proxy ${proxy}` : ""}...`,
    imageConfig,
  );
...
async function generateWithImagen(
  ...
  console.log(
    `Generating image with Imagen${proxy ? ` via proxy ${proxy}` : ""}...`,
    { imageSize: imageSize === "4K" ? "2K" : imageSize, aspectRatio: args.aspectRatio },
  );
  const response = await postGoogleJson<{
    predictions?: Array<any>;
    generatedImages?: Array<any>;
