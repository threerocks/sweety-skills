...
```bash
npx -y bun skills/sweety-markdown-to-html/scripts/main.ts article.md --theme newspaper --no-open
```

生成主题画廊：

```bash
npx -y bun skills/sweety-markdown-to-html/scripts/main.ts article.md --gallery --recommend newspaper magazine ink --no-open
```
