...
# 使用主题
${BUN_X} ./scripts/wechat-article.ts --markdown article.md --theme newspaper

# 兼容旧参数；当前由 sweety-markdown-to-html 统一处理外链脚注
...
| `--theme <name>` | 主题：`sweety-markdown-to-html` 的 30 套主题，兼容 `default/grace/simple/modern` 旧别名 |
| `--no-cite` | 兼容参数，当前不单独改变外链脚注行为 |
...
Markdown 模式由 `sweety-markdown-to-html` 统一渲染，默认将普通外链整理为文末参考链接，以适配微信阅读体验。
...
| `md-to-wechat.ts` | 兼容适配层：调用 `sweety-markdown-to-html` 转 HTML（含占位符） |
