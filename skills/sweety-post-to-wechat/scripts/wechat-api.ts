...
async function fetchAccessToken(appId: string, appSecret: string): Promise<string> {
  const url = `${apiUrl("/cgi-bin/token")}?grant_type=client_credential&appid=${appId}&secret=${appSecret}`;
...
async function uploadToWechat(
  ...
  const uploadPath = uploadType === "body" ? "/cgi-bin/media/uploadimg" : "/cgi-bin/material/add_material";
  const url = `${apiUrl(uploadPath)}?type=image&access_token=${accessToken}`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
...
async function publishToDraft(
  ...
  const url = `${apiUrl("/cgi-bin/draft/add")}?access_token=${accessToken}`;
...
function printUsage(): never {
  console.log(`通过 API 发布文章到微信公众号草稿箱

用法:
  npx -y bun wechat-api.ts <file> [options]

参数:
  file                Markdown (.md)、HTML (.html) 文件；--type newspic 时也可传图片目录

选项:
  --type <type>       文章类型: news (文章, 默认) 或 newspic (图文)
  --title <title>     覆盖标题
  --content <text>    newspic 图片目录模式下的贴图正文
  --author <name>     作者名（最多 16 字）
  --summary <text>    文章摘要（最多 128 字）
...
示例:
  ...
  npx -y bun wechat-api.ts images/ --type newspic --title "图册" --content "图册说明"
...
function listImageFiles(dir: string): string[] {
  ...
  return fs.readdirSync(dir)
    .filter((entry) => supported.test(entry))
...
async function main(): Promise<void> {
  ...
  let newspicImages: string[] = [];
  const sourceStat = fs.statSync(filePath);

  if (sourceStat.isDirectory()) {
    if (args.articleType !== "newspic") {
      console.error("错误: 只有 --type newspic 支持图片目录输入。文章发布请提供 Markdown 或 HTML 文件。");
      process.exit(1);
    }

    newspicImages = listImageFiles(filePath);
    if (newspicImages.length === 0) {
      console.error(`错误: 图片目录中没有可用图片: ${filePath}`);
      process.exit(1);
    }
    if (newspicImages.length > 20) {
      console.error(`错误: newspic 图片消息最多支持 20 张图片，当前 ${newspicImages.length} 张。`);
      process.exit(1);
    }
  ...
  if (args.articleType === "newspic" && newspicImages.length > 0 && !htmlContent.trim()) {
    console.error("错误: 图片目录贴图需要提供正文。请使用 --content。");
    process.exit(1);
  }
  ...
  if (extConfig.base_url) {
    configuredBaseUrl = extConfig.base_url.replace(/\/+$/, "");
    console.error(`[wechat-api] 使用自定义 API 地址: ${configuredBaseUrl}`);
  }
  const resolved = resolveAccount(extConfig, args.account);
  if (resolved.name) console.error(`[wechat-api] 账号: ${resolved.name} (${resolved.alias})`);
  ...
  if (args.dryRun) {
    console.log(JSON.stringify({
      ...
      imageCount: newspicImages.length || undefined,
      images: newspicImages.length > 0 ? newspicImages : undefined,
  ...
  if (newspicImages.length > 0) {
    console.error("[wechat-api] 正在上传贴图图片素材...");
    for (const imagePath of newspicImages) {
      const materialResp = await uploadImage(imagePath, accessToken, baseDir, "material");
      if (materialResp.media_id) imageMediaIds.push(materialResp.media_id);
    }
  } else {
    console.error("[wechat-api] 正在上传正文图片...");
    const processed = await uploadImagesInHtml(
      htmlContent,
      accessToken,
      baseDir,
      contentImages,
    ...
    firstCoverMediaId = processed.firstCoverMediaId;
    imageMediaIds = processed.imageMediaIds;
  }
  ...
  if (args.articleType === "news" && !thumbMediaId) {
    console.error("错误: 缺少封面图片。请通过 --cover、frontmatter.coverImage 或在内容中包含图片。");
    process.exit(1);
  }

  if (args.articleType === "newspic" && imageMediaIds.length === 0) {
    console.error("错误: 图文类型至少需要一张内容图片。");
    process.exit(1);
  }
  ...
  console.error(`[wechat-api] 发布成功! media_id: ${result.media_id}`);
}
