...
选项:
  --title <text>     文章标题（自动从 Markdown 提取）
  --content <text>   文章内容（与 --image 配合使用）
  --html <path>      HTML 文件（替代 --content）
  --markdown <path>  Markdown 文件（推荐）
  --theme <name>     Markdown 主题（支持 wechat-markdown-to-html 的 30 套主题和 legacy alias）
  --color <name|hex> 主色调 (blue, green, vermilion 等或十六进制值)
  --no-cite          禁用 Markdown 模式下外部链接的底部引用
  --author <name>    作者名
  --summary <text>   文章摘要
  --image <path>     内容图片，可重复（仅与 --content 配合使用）
  --submit           保存为草稿
  --profile <dir>    Chrome 配置目录
  --account <alias>  按别名选择账号（多账号设置时使用）
  --cdp-port <port>  连接已有的 Chrome 调试端口（不启动新实例）

示例:
  npx -y bun wechat-article.ts --markdown article.md
  npx -y bun wechat-article.ts --markdown article.md --theme elegant-blue --submit
  npx -y bun wechat-article.ts --markdown article.md --theme default --color '#A93226'
...
Markdown 模式:
  Markdown 中的图片被转为占位符。粘贴 HTML 后，逐个选中占位符、
  滚动到可见区域、删除并替换为从剪贴板粘贴的实际图片。外部链接
  默认转为底部引用。
`);
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  if (resolved.name) console.log(`[wechat] 账号: ${resolved.name} (${resolved.alias})`);
  ...
  if (!markdownFile && !htmlFile && !title) { console.error('错误: 需要 --title（或使用 --markdown/--html）'); process.exit(1); }
  if (!markdownFile && !htmlFile && !content) { console.error('错误: 需要 --content、--html 或 --markdown'); process.exit(1); }
