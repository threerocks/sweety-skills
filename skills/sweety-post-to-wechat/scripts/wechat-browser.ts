...
async function main(): Promise<void> {
  ...
  if (resolved.name) console.log(`[wechat-browser] 账号: ${resolved.name} (${resolved.alias})`);
  ...
  if (!markdownFile && !title) {
    console.error('错误: 需要 --title 或 --markdown');
    process.exit(1);
  }
  if (!markdownFile && !content) {
    console.error('错误: 需要 --content 或 --markdown');
    process.exit(1);
  }
  if (images.length === 0 && !imagesDir) {
    console.error('错误: 需要 --image 或 --images');
    process.exit(1);
  }
