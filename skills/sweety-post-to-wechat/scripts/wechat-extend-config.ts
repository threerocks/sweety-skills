...
function parseWechatExtend(content: string): WechatExtendConfig {
  ...
  for (const raw of lines) {
    ...
    switch (key) {
      ...
      case "base_url": config.base_url = val; break;
    }
  }
...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const paths = [
    path.join(process.cwd(), ".sweety-skills", "sweety-post-to-wechat", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"),
      "sweety-skills", "sweety-post-to-wechat", "EXTEND.md"
    ),
    path.join(os.homedir(), ".sweety-skills", "sweety-post-to-wechat", "EXTEND.md"),
  ];
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or .sweety-skills/.env file." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const cwdEnvPath = path.join(process.cwd(), ".sweety-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".sweety-skills", ".env");
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<cwd>/.sweety-skills/.env (${prefixedKeyLabel})`, cwdEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
      buildCredentialSource(`~/.sweety-skills/.env (${prefixedKeyLabel})`, homeEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("<cwd>/.sweety-skills/.env", cwdEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("~/.sweety-skills/.env", homeEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
